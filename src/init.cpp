...
void Initializer::create_window(std::set<int>* err) {
  this->w =
      SDL_CreateWindow("Synth", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, BWIDTH, BHEIGHT, 0);
  if (!this->w) {
...
void Initializer::create_renderer(std::set<int>* err) {
  this->r = SDL_CreateRenderer(this->w, -1, SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC);
  if (!this->r) {
